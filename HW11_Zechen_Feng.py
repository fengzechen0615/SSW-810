...
class Repository:
    ...
    def __init__(self, path):
        ...
        self._get_instructor_table_db(os.path.join(path, 'SSW-810.db'))
        ...
        try:
            for cwid, name, major_name in file_reading_gen(path, 3, sep='\t', header=True):
                try:
                    ...
                    print(f"Found unknown major {major_name}")
    ...
    def _get_instructor_table_db(self, db_path):
        try:
            database = sqlite3.connect(db_path)
            sql = """select I.CWID, I.Name, I.Dept, G.Course, count(*) as NumberOfStudents
    ...
    def instructor_database_pretty_table(self):
        """ instructor database pretty table"""
        pretty_table = PrettyTable()
        pretty_table.field_names = Instructor.PT_FIELDS
        for line in self._data:
            pretty_table.add_row(line)
        return pretty_table
...
if __name__ == '__main__':
    ...
    print("Instructor Database Summary")
    print(STEVENS.instructor_database_pretty_table())
